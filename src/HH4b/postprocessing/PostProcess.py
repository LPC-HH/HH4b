...
def _get_mass_windows(args) -> tuple[dict, dict]:
    ...
    fom = {"H2Msd": [110, 140]}
    blind = {"H2Msd": [110, 140]}
    # (fom_lo, fom_hi, blind_lo, blind_hi) per tagger
    pnet_windows = {
...
def _setup_shape_var(args, blind_window_by_mass: dict) -> ShapeVar:
    """Build the ShapeVar used for template histogram binning."""
    n_bins = int((220 - 60) / args.mass_bins)
    return ShapeVar(
        args.mass,
...
def _combine_years(
    ...
    """
    if len(args.years) == 1:
        return events_dict[args.years[0]], {}, {}

    # Only Run3 22/23 MC is available; scale to whatever years are requested.
...
def _build_combined_cutflow(
    ...
    """Assemble and print the combined cutflow table across all years.

    ABCD estimates for Bin 1 and VBF are appended as extra rows so the table
    shows the expected background yield alongside the raw event counts.
    """
    if not args.years:
        return None
    ...
    # ABCD background estimate (no VBF-region veto applied here)
    ...
    # Use 2022EE as the reference year for the cut list when available
...
def _run_fom_scans(args, events_combined: dict, mass_window, plot_dir: Path, bg_keys: list) -> None:
    """Run FoM scans for VBF, Bin 1, and/or Bin 2 according to CLI flags."""
    if not args.fom_scan:
        return

    if args.fom_scan_vbf and args.vbf:
        label = "Scanning VBF WPs" + ("" if args.vbf_priority else ", vetoing Bin1")
        print(label)
    ...
    if args.fom_scan_bin2:
        if args.vbf:
            print(
                f"Scanning Bin 2, vetoing VBF TXbb WP: {args.vbf_txbb_wp}"
                f" BDT WP: {args.vbf_bdt_wp},"
                f" bin 1 WP: {args.txbb_wps[0]} BDT WP: {args.bdt_wps[0]}"
            )
        else:
...
def _save_cutflows(
    args, templ_dir: Path, cutflows: dict, cutflow_combined: pd.DataFrame | None
) -> None:
    """Write per-year and combined preselection cutflow CSVs."""
    for year in args.years:
...
def _make_templates(
    ...
    """Build and save per-year hist.Hist templates for all JEC/JMR shifts."""
    for year in args.years:
        (templ_dir / year).mkdir(parents=True, exist_ok=True)
        templates = {}
        for jshift in [""] + hh_vars.jec_shifts + hh_vars.jmsr_shifts:
            events_by_year = {s: ev[ev["year"] == year] for s, ev in events_combined.items()}
...
def _save_event_lists(args, events_dict: dict) -> None:
    """Write per-year ROOT event lists for data and signal samples."""
    eventlist_columns = [
        "event",
        "bdt_score",
        "bdt_score_vbf",
    ...
    for year, year_dict in events_dict.items():
        ...
        for key, tree_df in year_dict.items():
            if not ("data" in key or "hh4b" in key or "vbfhh4b" in key):
                continue
            array_to_save = {col: tree_df[col].to_numpy() for col in eventlist_columns}
...
def postprocess_run3(args):
    ...
    # ── 8. Save event lists ───────────────────────────────────────────────────
    if args.event_list:
        _save_event_lists(args, events_dict)
