...
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    files = {
        "0-signal" : path_0+"mc/parts/GluGlutoHHto4B_kl-1p00_kt-1p00_c2-0p00_TuneCP5_13p6TeV_powheg-pythia8_tree.root",
    }
    ...
    for s in list(files.keys()):
        ...
        df[s] = df[s].Define("dHiggsDeltaRegMass", "sqrt((dHH_H1_regmass-125.0)*(dHH_H1_regmass-125.0)+(dHH_H2_regmass-120.0)*(dHH_H2_regmass-120.0))")
        if "data" in s:
            df[s] = df[s].Define("weight", "1")
        else:
            df[s] = df[s].Define("weight", "genWeight*xsecWeight*lumiwgt*btagSF_central*trgSF_HLT_QuadPFJet70_50_40_35_PFBTagParticleNet_2BTagSum0p65_central*puWeight")

        # Apply filters here:
        df[s] = df[s].Filter("pass_resolved_skim", "Resolved skim")
        df[s] = df[s].Filter("passTrig_HLT_QuadPFJet70_50_40_35_PFBTagParticleNet_2BTagSum0p65", "Trigger")
        df[s] = df[s].Filter("passL1unprescaled_HLT_QuadPFJet70_50_40_35_PFBTagParticleNet_2BTagSum0p65", "L1 unprescaled")
        df[s] = df[s].Filter("passTrigObjMatching_HLT_QuadPFJet70_50_40_35_PFBTagParticleNet_2BTagSum0p65", "Trigger matching")
        df[s] = df[s].Filter("passmetfilters", "MET filters")
        df[s] = df[s].Filter("passjetvetomap", "Passed jet veto map")

        print("Event counts for resolved, after applying resolved skim : ", df[s].Count().GetValue())
        ...
        df[f"{s}-4b-CR"] = df[f"{s}-4b"].Filter("dHiggsDeltaRegMass >= 30.0 && dHiggsDeltaRegMass < 55", "4b CR")
        df[f"{s}-3b-CR"] = df[f"{s}-3b"].Filter("dHiggsDeltaRegMass >= 30.0 && dHiggsDeltaRegMass < 55", "3b CR")

        df[f"{s}-4b-SR-wboosted"] = df[f"{s}-4b"].Filter("pass_boosted_skim", "Boosted skim")
        df[f"{s}-4b-SR-wboosted"] = df[f"{s}-4b-SR-wboosted"].Filter("n_ak8 >= 2", "At least two fatjets")
        df[f"{s}-4b-SR-wboosted"] = df[f"{s}-4b-SR-wboosted"].Filter("ak8_pt[0] > 300", "Leading AK8 with pT > 300")
        print("Counter for events passing both resolved and boosted skims : ", df[f"{s}-4b-SR-wboosted"].Count().GetValue())
        ...
        for region in regions:
            for var in variables:
                if "data" in sample and "SF" in var:
                    continue
                h          = ROOT.RDF.TH1DModel("%s_%s_%s" % (sample, region, var), labels[var], len(bins[var])-1, array.array("d",  bins[var]))
                h_weighted = ROOT.RDF.TH1DModel("%s_%s_%s_weighted" % (sample, region, var), labels[var], len(bins[var])-1, array.array("d",  bins[var]))
                histos["%s-%s-%s" % (sample, region, var)]          = df["%s-%s" % (sample, region)].Histo1D(h, var)
                histos["%s-%s-%s_weighted" % (sample, region, var)] = df["%s-%s" % (sample, region)].Histo1D(h_weighted, var, "weight")
        ...
        for region in regions:
            for var in variables:
                ...
                if "data" in sample and "SR" in region and "4b" in region:
                    ...
                    plot(histos["0-%s-%s-%s" % (sample, region, var)],
                         "Debug_%s_%s_%s.pdf" % (sample, region, var), sample, region, False)
                    
                    plot(histos["0-%s-%s-%s_weighted" % (sample, region, var)],
                         "Debug_%s_%s_%s_weighted.pdf" % (sample, region, var), sample, region, True)
