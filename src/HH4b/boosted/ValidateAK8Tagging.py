...
def load_events(path_to_dir, year, jet_collection, pt_cut, msd_cut, jet_coll_pnet, match_higgs):
    ...
    sample_dirs_sig = {
        year: {
            "hh4b": ["GluGlutoHHto4B_kl-1p00_kt-1p00_c2-0p00_TuneCP5_13p6TeV?"], # the ? enforces exact matching
    ...
    events_dict = {
        ...
        **utils.load_samples(
            ...
            columns=utils.format_columns(columns), # columns to load from parquet (to not load all columns), IMPORTANT columns must be formatted: ("column name", "idx")
...
def get_roc_inputs(events_dict, jet_collection, discriminator_name, jet_index, sig_key="hh4b", bg_keys=["qcd"]):
...
def get_roc(events_dict, jet_collection, discriminator_name, discriminator_label, discriminator_color, jet_indices, pt_cut, msd_cut, mreg_cut = None):
    y_true_arr = []; scores_arr = []; weights_arr = [];
    for jet_index in jet_indices:
        events_dict_masked = {key: events[
        ...
        y_true_i, scores_i, weights_i = get_roc_inputs(events_dict_masked, jet_collection, discriminator_name, jet_index)
...
def main(args):
    ...
    legtitle = rf"{pt_cut[0]} < $p_T$ < {pt_cut[1]} GeV"+"\n"+f" {msd_cut[0]}"+r" < $m_{SD}$ <"+f" {msd_cut[1]} GeV"
    ...
    events_dict = load_events(path_to_dir, year, jet_collection, pt_cut, msd_cut, jet_coll_pnet, match_higgs)

    for jet_indices in [[0,1], [0], [1]]:
        rocs = {
            "PNetTXbbLegacy": get_roc(events_dict, jet_collection, "PNetTXbbLegacy", "ParticleNet Legacy Hbb vs QCD", "blue", jet_indices, pt_cut, msd_cut, mreg_cut),
            "PNetTXbb": get_roc(events_dict, jet_collection, "PNetTXbb", "ParticleNet 103X Hbb vs QCD", "orange", jet_indices, pt_cut, msd_cut, mreg_cut),
            "ParTTXbb": get_roc(events_dict, jet_collection, "ParTTXbb", "GloParTv2 Hbb vs QCD", "red", jet_indices, pt_cut, msd_cut, mreg_cut)
        }
        ...
        plotting.multiROCCurveGrey(
            ...
            title=f"AK8 Jets {jet_indices}" if jet_collection == "ak8FatJet" else f"bb Jets {jet_indices}",
            legtitle=legtitle,
