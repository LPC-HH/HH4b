    ...
    for xbb_cut in xbb_cuts:
        for key in ["qcd", "ttbar", "vhtobb", "vjets", "hh4b"]:
            ...
            if key in msd_dict:
                ...
                h2_mass = events["bbFatJetPNetMass"].to_numpy()[:, 1]
                h2_msd = events["bbFatJetMsd"].to_numpy()[:, 1]
                h2_xbb = events["bbFatJetPNetXbb"].to_numpy()[:, 1]
...
def main(args):
    ...
    (model_dir / "inferences" / args.year).mkdir(exist_ok=True, parents=True)
    ev_hh4b = ev_test[ev_test.index.get_level_values(0) == "hh4b"]
    ev_qcd = ev_test[ev_test.index.get_level_values(0) == "qcd"]
    ev_tt = ev_test[ev_test.index.get_level_values(0) == "ttbar"]
    np.save(f"{model_dir}/inferences/{args.year}/evt_hh4b.npy", ev_hh4b)
    np.save(f"{model_dir}/inferences/{args.year}/evt_qcd.npy", ev_qcd)
    np.save(f"{model_dir}/inferences/{args.year}/evt_ttbar.npy", ev_tt)
