...
def main(args):
    ...
    expected_soverb_by_xbb_cut_by_bdt_cut = {}
    expected_s_by_xbb_cut_by_bdt_cut = {}
    expected_b_by_xbb_cut_by_bdt_cut = {}
    ...
    for xbb_cut in xbb_cuts:
        expected_soverb_by_xbb_cut_by_bdt_cut[xbb_cut] = {}
        expected_s_by_xbb_cut_by_bdt_cut[xbb_cut] = {}
        expected_b_by_xbb_cut_by_bdt_cut[xbb_cut] = {}
        ...
        for bdt_cut in bdt_cuts:
            ...
            expected_soverb_by_xbb_cut_by_bdt_cut[xbb_cut][bdt_cut] = soversb
            expected_s_by_xbb_cut_by_bdt_cut[xbb_cut][bdt_cut] = nevents_sig
            expected_b_by_xbb_cut_by_bdt_cut[xbb_cut][bdt_cut] = nevents_bkg
        ...
        print(f"Expected sensitivity by BDT cut {expected_soverb_by_xbb_cut_by_bdt_cut[xbb_cut]}")
        # print(f"Expected S by BDT cut {expected_s_by_xbb_cut_by_bdt_cut[xbb_cut]}")
    ...
    if len(bdt_cuts_data) > 0:
        ...
        print(
            f"From Data: Xbb:{optimal_xbb_cut_data:.3f} BDT:{optimal_bdt_cut_data:.2f} S/(S+B):{sensitivity_data:.2f} S: {expected_s_by_xbb_cut_by_bdt_cut[optimal_xbb_cut_data][optimal_bdt_cut_data]:.2f}"
        )
    ...
    for itoy in range(ntoys):
        ...
        if len(bdt_cuts_toys) > 0:
            ...
            print(
                f"Optimal {optimal_xbb_cut:.3f} {optimal_bdt_cut:.2f}, FOM: {biggest} {figure_of_merit_toys[biggest]:.2f}, Difference: {(figure_of_merit_toys[biggest]-compare_to):.2f}, Expected S/S+B: {compare_to:.2f}, Expected S: {compare_to_s:.2f}"
            )
            ...
            print("Optimal cuts: Xbb ", optimal_xbb_cut, "BDT ", optimal_bdt_cut)
    ...
    def plot_h(h_hist, xlabel, plot_name, xlim, gaus_label):
        ...
        hep.histplot(
            h_hist,
            ax=ax,
            label=r"Expected S/$\sqrt{S+B}$ from optimal cut in data: "
